...
class TpdfInvoice
{
    ...
    {
       /* $font1 = File::get(public_path('fonts/HelveticaMedCd.ttf'));
        $font2 = File::get(public_path('fonts/HelveticaNeue-LightCond.ttf'));
        $fontName = 'Helvetica';
        $this->AddFont($fontName,'', $font1, false);
        $this->AddFont($fontName,'B', $font2, false);
        ...
        $this->Cell(100,20, "Some UTF-8 String");*/
    ...
    public function Header()
    {
        ...
        $positionX = $this->document['w'] - $this->margins['l'] - $this->margins['r'] - max(mb_strtoupper($this->GetStringWidth(trans('invoice.number'), self::ICONV_CHARSET_INPUT)),
                mb_strtoupper($this->GetStringWidth(trans('invoice.date'), self::ICONV_CHARSET_INPUT)),
                mb_strtoupper($this->GetStringWidth(trans('invoice.due'), self::ICONV_CHARSET_INPUT))) - 35;
        ...
        if (! empty($this->time)) {
            ...
            $this->SetFont($this->font, 'B', 9);
            $this->SetTextColor($this->color[0], $this->color[1], $this->color[2]);
            $this->Cell(32, $lineheight, $this->str_iconv(trans('invoice.time'), true) . ':', 0, 0,
                'L');
            $this->SetTextColor(50, 50, 50);
            $this->SetFont($this->font, '', 9);
        ...
        if (! isset($this->productsEnded)) {
            ...
            $this->Cell($this->firstColumnWidth, 10, $this->str_iconv(trans('invoice.description'), true),
                0, 0, 'L', 0);
            $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
            $this->Cell($width_other, 10, $this->str_iconv(trans('invoice.qty'), true), 0, 0, 'C', 0);
            if (isset($this->vatField)) {
                $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
                $this->Cell($width_other, 10, $this->str_iconv(trans('invoice.vat'), true), 0, 0, 'C',
                    0);
            }
            $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
            ...
            if (isset($this->discountField)) {
                $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
                $this->Cell($width_other, 10, $this->str_iconv(trans('invoice.discount'), true), 0, 0,
                    'C', 0);
            }
            $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
    ...
    public function Body()
    {
            ...
            foreach ($this->items as $item) {
                ...
                if ($item['description']) {
                    ...
                    $calculateHeight->MultiCell($this->firstColumnWidth, 3,
                        $this->str_iconv($item['description']), 0, 'L', 1);
                ...
                $x = $this->GetX();
                $this->Cell($this->firstColumnWidth, $cHeight, $this->str_iconv($item['item']), 0, 0, 'L',
                    1);
                if ($item['description']) {
                    $resetX = $this->GetX();
                    $resetY = $this->GetY();
                    ...
                    $this->MultiCell($this->firstColumnWidth, floor($this->fontSizeProductDescription/2), $this->str_iconv($item['description']), 0,
                        'L', 1);
                    //Calculate Height
                    $newY    = $this->GetY();
        ...
        if (isset($this->signature) and !empty($this->signature)) {
            $this->Image($this->signature, $this->signatureX, $this->signatureY, $this->dimensions[0],
                $this->dimensions[1]);
        }
    ...
    public function Footer()
    {
        ...
        $this->Cell(0, 10, iconv('UTF-8', 'ISO-8859-1', trans('invoice.page')) . ' ' . $this->PageNo() . ' ' . trans('invoice.page_of') . ' {nb}', 0, 0,
            'R');
    }
