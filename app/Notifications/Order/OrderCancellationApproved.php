...
class OrderCancellationApproved extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        ...
        ->subject(trans('notifications.cancellation_request_approved.subject', ['order' => $this->order->order_number]))
        ->markdown('admin.mail.order.cancellation_approved', ['url' => url('/'), 'order' => $this->order]);
    }
    ...
    public function toArray($notifiable)
    {
        return [
            ...
            'subject' => trans('notifications.cancellation_request_approved.subject', ['order' => $this->order->order_number]),
            'message' => trans('notifications.cancellation_request_approved.message', ['order' => $this->order->order_number]),
        ];
    }
}
